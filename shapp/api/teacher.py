...
def edit_teacher_profile():
    ...
    if tmp is not None and tmp is not t:
        return jsonify({'msg': '电话已注册，不能使用此电话!'}), 403
...
def get_teacher_profile():
    ...
    return jsonify(t.teacher_format_info()), 200
...
def mainteacher_signup():
    ...
    if wid is None or password is None or name is None:
        return jsonify({
            "msg": "need wid passwod name"
        })
...
def teacher_addclass(classid):
    ...
    if wid is None or password is None or name is None or kind is None:
        return jsonify({
            "msg": "need wid password name and subject"
        })
